...
class Maze:
    def __init__(self, size, start):     # start是一个元组
        ...
        self.maze = np.zeros((self.r_num, self.c_num, 2), dtype=np.uint8)    # 0: 0 是墙 1是路 1: 1是已经被访问过
    ...
    def move(self):
        ...
        if self.c > 0 and self.maze[self.r, self.c - 1, 1] == 0:     # 判断左方是否可以走
            directions.append('L')
        if self.r > 0 and self.maze[self.r - 1, self.c, 1] == 0:     # 判断上方是否可以走
            directions.append('U')
        if self.c < self.c_num - 1 and self.maze[self.r, self.c + 1, 1] == 0:     # 判断右方是否可以走
            directions.append('R')
        if self.r < self.r_num - 1 and self.maze[self.r + 1, self.c, 1] == 0:     # 判断下方是否可以走
            directions.append('D')
        return directions

    def opposite(self, next_point):
        dx = next_point[0] - self.r
        dy = next_point[1] - self.c
        x = next_point[0] + dx
        y = next_point[1] + dy
        if 0 <= x < self.r_num and 0 <= y < self.c_num:     # 判断对面的点在棋盘中
